...
garch_model = arch_model(returns_train, vol='GARCH', p=1, q=1,o=1, mean='Zero')
...
for i in range(len(returns_test)):
    ...
    model = arch_model(rolling_window, vol='GARCH', p=1, q=1,o=1, mean='Zero',dist='Normal')
...
# 5. 保存预测结果
forecast_df.to_csv(f'{symbol}_gjr_garch_forecast.csv', date_format='%Y-%m-%d', index_label='Date')