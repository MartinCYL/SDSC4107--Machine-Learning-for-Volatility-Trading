...
garch_model = arch_model(returns_train, vol='EGARCH', p=1, q=1, o=0)
...
rolling_window= rolling_window.iloc[-1000:]
...
for i in range(len(returns_test)):
    ...
    model = arch_model(rolling_window, vol='EGARCH', p=1, q=1, o=0)
...
# 5. 保存预测结果
forecast_df.to_csv(f'{symbol}_garch_forecast.csv', date_format='%Y-%m-%d', index_label='Date')